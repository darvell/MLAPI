...
public sealed class ClassTypeReferencePropertyDrawer : PropertyDrawer
{
    ...
    {
        if (filter is ClassImplementsAttribute interfaceFilter)
        {
            return AppDomain.CurrentDomain.GetAssemblies().SelectMany(x => x.GetTypes())
                .Where(x => interfaceFilter.InterfaceType.IsAssignableFrom(x) && x.IsClass).ToList();
        }
        ...
        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
        {
            ...
            foreach (var referencedAssembly in assembly.GetReferencedAssemblies())
            {
                ...
                processedAssemblies.Add(referencedAssembly.FullName);
            }
        }
